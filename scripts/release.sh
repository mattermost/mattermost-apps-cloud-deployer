...
release-notes --org mattermost --repo mattermost-apps-cloud-deployer --start-sha "${LATEST_RELEASE_NEXT_COMMIT_SHA}" --end-sha "${FUTURE_RELEASE_SHA}"  --output ./build/_output/docs/relnote.md --required-author "" --branch master
...
echo "The image for this release is \`docker.io/mattermost/mattermost-apps-cloud-deployer:${GITHUB_REF_NAME}\`" >> ./build/_output/docs/relnote_parsed.md
echo "\n" >> ./build/_output/docs/relnote_parsed.md
